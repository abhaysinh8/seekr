...
import { dcgAtK, evaluateConfigurations, formatEvaluationTable, ndcgAtK, precisionAtK, recallAtK, reciprocalRank } from './evaluation.js';
...
describe('offline ranking evaluation', () => {
  ...
  it('compares TF-IDF, BM25 parameters, and field weights', () => {
    ...
    const results = evaluateConfigurations(dataset, [
      { name: 'TF-IDF', search: { ranking: 'tfidf' } },
      { name: 'BM25 tuned', search: { ranking: 'bm25', k1: 1.8, b: 0.5 } },
      { name: 'Title boost', index: { fields: { title: { searchable: true, weight: 4 }, body: { searchable: true } } } },
    ], 2);
